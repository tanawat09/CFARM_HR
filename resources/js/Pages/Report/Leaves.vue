...
watch(selectedMonth, (newVal) => {
    if (newVal !== props.filters.month) {
        router.get(route('reports.leaves'), { month: newVal }, {
            preserveState: true,
            preserveScroll: true,
            replace: true
        });
...
const exportExcel = () => {
    window.location.href = route('reports.leaves.export', { month: selectedMonth.value });
};
...
const getLeaveTypeBadge = (type) => {
    ...
    return map[type] || { class: 'bg-indigo-100 text-indigo-700' };
};
    ...
    <AuthenticatedLayout>
             ...
             <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl w-max shadow-inner">
                        <Link :href="route('reports.leaves')" class="flex items-center gap-2 px-5 py-2.5 text-sm font-black rounded-xl transition-all bg-white text-indigo-700 shadow-md ring-1 ring-slate-900/5">
        ...
        <div class="py-8">
            <div class="px-4 sm:px-6 lg:px-8 space-y-8">
                <!-- KPI Widgets grid -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 relative z-10">
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-hidden relative group hover:-translate-y-1 transition duration-300">
                        ...
                        <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 relative z-10">อนุมัติเรียบร้อยในเดือนนี้</h4>
                        <div class="text-3xl font-black text-emerald-600 relative z-10">{{ stats.approved }}<span class="text-sm text-slate-400 font-medium ml-1">คำขอ</span></div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-hidden relative group hover:-translate-y-1 transition duration-300">
                        ...
                        <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 relative z-10">รอการอนุมัติ</h4>
                        <div class="text-3xl font-black text-amber-500 relative z-10">{{ stats.pending }}<span class="text-sm text-slate-400 font-medium ml-1">คำขอ</span></div>
                        <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 to-amber-400"></div>
                    </div>

                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 overflow-hidden relative group hover:-translate-y-1 transition duration-300">
                         <div class="absolute -right-4 -bottom-4 text-rose-50 w-24 h-24 transform group-hover:scale-110 transition">
                        ...
                        <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 relative z-10">ไม่อนุมัติ / ยกเลิก</h4>
                        <div class="text-3xl font-black text-rose-500 relative z-10">{{ stats.rejected }}<span class="text-sm text-slate-400 font-medium ml-1">คำขอ</span></div>
                    ...
                    <div v-if="chartData.length > 0" class="flex flex-col gap-4">
                        <div v-for="item in chartData" :key="item.type" class="flex items-center gap-4">
                            ...
                            <div class="flex-1 bg-slate-100 rounded-full h-4 overflow-hidden relative">
                                <div class="absolute top-0 left-0 bottom-0 bg-indigo-500 transition-all duration-1000 ease-out" 
                                     :style="{ width: `${(item.count / totalChartLeaves) * 100}%` }">
                                </div>
                            </div>
                            <div class="w-12 shrink-0 text-right font-bold text-indigo-600">{{ item.count }} <span class="text-xs text-slate-400">ครั้ง</span></div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto w-full">
                        <table class="w-full text-left whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-slate-50 text-sm">
                                <tr v-for="leave in leaves.data" :key="leave.id" class="hover:bg-indigo-50/30 transition group">
                                    <td class="px-6 py-4">
