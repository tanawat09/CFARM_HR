...
const form = useForm({
    ...
    code: props.worksite.code || '',
    address: props.worksite.address || '',
    branch_id: props.worksite.branch_id || '',
    latitude: Number(props.worksite.latitude) || '',
    longitude: Number(props.worksite.longitude) || '',
...
const submit = () => {
    form.put(route('worksites.update', props.worksite.id));
};
</script>

<template>
    <Head title="แก้ไขข้อมูลสาขา" />
        ...
        <template #header>
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="font-bold text-2xl text-slate-800 leading-tight tracking-tight">🏢 แก้ไขข้อมูลสาขา: {{ worksite.code }}</h2>
                </div>
            </div>
        </template>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" v-model="form.code" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 uppercase" required>
                                ...
                                <div>
                                    ...
                                    <input type="text" v-model="form.name" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    <InputError :message="form.errors.name" class="mt-2" />
                                </div>

                                <div>
                                    <label class="block font-bold text-sm text-slate-700 mb-2">ที่อยู่ / รายละเอียด</label>
                                    <textarea v-model="form.address" rows="3" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 resize-none"></textarea>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="bg-indigo-50 rounded-2xl p-5 border border-indigo-100 relative overflow-hidden">
                                    ...
                                    <div class="flex justify-between items-center mb-4">
                                        <label class="font-bold text-sm text-indigo-900">อัพเดตพิกัด</label>
                                        <button type="button" @click="getCurrentLocation" :disabled="isLocating" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition shadow-sm disabled:opacity-50">
                                            ...
                                            {{ isLocating ? 'กำลังค้นหา...' : 'ดึงตำแหน่งปัจจุบันใหม่' }}
                                        </button>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" step="any" v-model="form.latitude" class="w-full rounded-xl border-indigo-200 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm" required>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" step="any" v-model="form.longitude" class="w-full rounded-xl border-indigo-200 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 font-mono text-sm" required>
